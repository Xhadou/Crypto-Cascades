...
NetworkX (pure Python) cannot compute triangle-based metrics (clustering, transitivity, small-world) on graphs with millions of nodes. The pipeline automatically selects the best available backend: **NetworKit** (parallel C++, fastest — `pip install networkit`), then **igraph** (single-threaded C — included in core dependencies), then NetworkX as last resort. For optimal performance on very large graphs, install NetworKit: `pip install networkit`. If the pipeline still hangs, reduce the `large_graph_nodes` threshold in `configs/config.yaml` under `thresholds` to force the fast path earlier. Eigenvector centrality and small-world coefficient are automatically skipped for graphs exceeding `large_graph_nodes` (default: 10,000).
